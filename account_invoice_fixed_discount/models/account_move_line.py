...
class AccountMoveLine(models.Model):
    ...
    @api.depends("quantity", "discount", "discount_fixed", "price_unit", "tax_ids", "currency_id")
    def _compute_totals(self):
        """Adjust the computation of the price_subtotal and price_total fields to
        account for the fixed discount amount.
        """
        # First, let parent compute for all lines
        res = super()._compute_totals()

        # Then recompute only for lines with fixed discount
        for line in self:
            if not float_is_zero(
                    line.discount_fixed, precision_rounding=line.currency_id.rounding
            ):
                # Calculate discount percentage from fixed discount
                discount = line._get_discount_from_fixed_discount()
                line_discount_price_unit = line.price_unit * (1 - (discount / 100.0))
    ...
    def _onchange_discount_fixed(self):
        """Compute the discount percentage based on the fixed discount amount."""
        if self.env.context.get("ignore_discount_onchange"):
            return
        ...
        for line in self:
            if not float_is_zero(
                    line.discount_fixed,
                    precision_rounding=(line.currency_id or line.company_id.currency_id).rounding
            ):
                line.discount = line._get_discount_from_fixed_discount()

    @api.onchange("discount")
    def _onchange_discount(self):
        """Clear fixed discount when percentage discount is changed manually."""
        if self.env.context.get("ignore_discount_onchange"):
            return

        for line in self:
            # Only clear fixed discount if discount field was manually changed
            if line.discount and not line.discount_fixed:
                continue
    ...
    def _get_discount_from_fixed_discount(self):
        ...
        # If fixed discount exceeds subtotal, reset both discounts
        if self.discount_fixed > subtotal:
            return 0.0

        # Calculate percentage: (fixed_discount / subtotal) * 100
        discount = (self.discount_fixed / subtotal) * 100.0
